...
<div class="page-header page-header-default border-top-lg border-top-primary">
	...
	<div class="breadcrumb-line">
		<ul class="breadcrumb">
			...
			<li><a href="{{base_url("superuser/post")}}">Artikel</a></li>
		...
		<div class="panel panel-flat">
			<div class="panel-body">
				<div class="form-group">
					<label><b class="text-danger position-left">*</b>Judul artikel : </label>
					<input type="text" name="title" value="{{@$table->title}}" class="form-control" placeholder="Ketikan judul artikel di sini ..." required="">
				</div>
				<div class="form-group">
	...
	<div class="col-md-4">
		<div class="panel panel-flat">
			<div class="panel-body">
				...
				<div class="form-group">
					<label><b class="text-danger position-left">*</b>Pilih Kategori : </label>
					<select class="form-control select2" name="category" required="">
						<option>Pilih Kategori</option>
						@foreach($category as $result)
							<option value="{{$result->id}}" {{match($result->id, @$table->category_id,"selected")}}>
								{{$result->name}}
							</option>
						@endforeach
					</select>
				...
				<div class="form-group">
					...
					<select class="form-control select2" name="tags[]" multiple="multiple">
						@foreach($tags as $result)
							...
							@else
							<option value="{{$result->id}}" {{matchArray(@$table->tags->pluck('tag_id')->toArray(),$result->id,true,'selected')}}>
							@endif
								{{$result->name}}
							</option>
						@endforeach

...
var el_courier_service  = $("input[name=courier_service]");
var el_courier_packet 	= $("#courier-packet");
var el_courier_cost 	= $("input[name=courier_cost]");
...
$("input[name=courier_custom]").change(function(){
	resetCourierData();

	var parent_default 			= $('#default-courier-box');
	var parent_custom 			= $('#custom-courier-box');
...
$(el_courier_service).change(function(){
	...
	if(province=="" || city=="" || district==""){
		ShowNotif('Opps!','Please Select Your Location First','bg-danger');
		resetCourierData();
		return;
	}
	...
	$.ajax({
		url:url_cost,
		method:'POST',
		data:{weight:weight,destination:district,courier:courier},
		beforeSend:function(){
			blockMessage(element,"Please Wait Getting data  . . .","#fff");
		}
	})
	.done(function(response){
		...
		$(el_courier_packet).html(html);
		$(el_courier_packet).prop('disabled',false);
		countCourierCost();
	})
	.fail(function(response){
		var response = response.responseJSON;
...
$(el_courier_packet).change(function(){
	...
	if($(this).val()==""){
		$(txt_courier_name).html('---');
		$(txt_courier_packet).html('---');
		$(txt_courier_estimation).html('---');
		$(txt_courier_cost).html('-,-');
		countCourierCost();
		return;
	}
	...
	$(txt_courier_packet).html(name);
	$(txt_courier_estimation).html(estimation+' hari');
	$(txt_courier_cost).html('Rp. '+parseInt(cost).formatMoney(0,',','.'))
